...
def validate_rule(rule: str, lineno: int, filename: str) -> list[str]:
    errors = []
                ...
                if val is None or val == "":
                    ...
                    if not pcre_has_closing_slash(val):
                        ...
                        if m:
                            ...
                            if compile_err:
                                err(f"'pcre' pattern failed to compile: {compile_err} — pattern: {val}")
...
def validate_file(filepath: str) -> list[str]:
    ...
    for lineno, raw in enumerate(lines, start=1):
        ...
        rule_errors = validate_rule(line, lineno, filepath)
...
def main(argv: list[str]) -> int:
    ...
    if all_errors:
        ...
        for e in all_errors:
            print(f"  ✗  {e}")
        print(f"\n{len(all_errors)} error(s) found across {len(argv)} file(s).")
        return 1

    print(f"[PASS] All rules validated successfully ({len(argv)} file(s)).")
    return 0
